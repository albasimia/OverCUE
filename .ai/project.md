...
複数Logical Deviceの演奏開始構成はGroup Presetとして保存する。Group PresetはLogical Device IDごとに参照Preset stable IDを持ち、Physical Device/session identityを保存しない。Group Preset適用後のCycle Presetは各デバイスの一時runtime stateとして維持する。
...
- Group Presetはstable ID、name、order、Logical Device→Preset stable ID assignmentを保持する。
...
- 既存v10 configにGroup Presetが無い場合はDefault Group Presetを補完する。v9以前からのmigration終端でも既存Logical Deviceを各Profileの先頭Presetへ割り当てる。
...
- Group Preset runtime適用は既存device-scoped controlを再利用し、同じGroup Preset assignmentのままCycle Presetした状態を通常statusで巻き戻さない。
...
- Group PresetとdevicePresetAssignmentsも3-way merge対象とする。
