...
設定version 3では `deviceProfiles` のキーにACK05の `PhysicalDeviceUniqueID`、値にプロファイル名を指定する。未登録デバイスは `defaultProfile` を使用し、初回入力時にその対応を設定ファイルへ保存する。シリアル番号が公開されない場合はDeviceAddress、LocationIDの順で識別子をフォールバックする。keyMapとchordMapには表示名ではなく安定したAction IDを保存する。version 1または2設定は原本を `config.vN.backup.json` へ保存し、内容を維持したままversion 3へ自動移行する。
