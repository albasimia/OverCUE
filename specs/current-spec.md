...
- Generic HID start restores main-runloop scheduling after every successful manager open (Close unschedules it), then explicitly copies currently connected interfaces. Snapshot and hotplug callbacks use the same idempotent registration/preload/state-resolution path. Config reload retries the snapshot; failed metadata stays fail-closed until lifecycle retry. Stop rejects callbacks and clears live interface/catalog/session state. Learn still captures on the shared runtime; no separate monitor or mapping-scope change is introduced.
